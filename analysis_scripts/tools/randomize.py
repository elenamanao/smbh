'''Library with functions for randomising sources in the Sky.
All the functions produce random r.a. in an interval between 
0 and 360 (degrees) with a uniform distribution.'''
...
def random_ra(ra_true, seed):
    """Produce random array of values for ra"""
    np.random.RandomState(seed)
    ra_rad_tmp = np.random.uniform(0,360, len(ra_true))

    return ra_rad_tmp

def mask_hotspot_and_randomize(ra_true, dec_true, seed, hs_ra, hs_dec, hs_radius):
    ...
    while i > 0:
        ...
        ra_tmp = random_ra(ra_true, seed)
        #check that no sources fall into PAO hotspot after permutation
        gcd = GreatCircleDistance(ra_tmp, dec_true,np.ones_like(ra_tmp)*hs_ra, np.ones_like(dec_true)*hs_dec, unit = 'deg')
...
def randomize_and_mask_hotspot(ra_true, dec_true, seed, hs_ra, hs_dec, hs_radius):
