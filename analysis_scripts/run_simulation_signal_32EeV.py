...
p.add_argument("--filter_sources", 
               type=str, 
               default='all_sources', 
               help="Which sources to analyse (all_sources, AGN, no_AGN)")
...
p.add_argument('--mode', 
               type=str, 
               default='sample', 
               help="sample: simulate public events, all_statistics: forecast full sample")
p.add_argument('--n_signal', 
               type=int, 
               default=0, 
               help="Number of signal events to be distributed among sources")
p.add_argument('--source_list', 
               type=str, 
               help="Path to the .npy or .csv file containing the source list with RA/Dec in radians")
p.add_argument('--mask_source', 
               type=str, 
               help="AGN or non-AGN")
...
if args.mask_source == 'non_AGN':
    ...
    sources = sources
    print(f"Distributing the events on all the sources.\nNumber of sources: {len(sources)}")
